...
import com.automation.ivr.store.DocumentStore;
...
public class VxmlScriptEngine {
    ...
    public Object executeScriptFile(String src) throws VxmlException {
        // StringBuilder script = new DocumentStore().getData(src);
        URI fullUrl = null;
        try {
            fullUrl = DocumentStore.getFullUri(src);
    ...
    public Object eval(String script) throws VxmlException {
        String scriptForEval = script;
        Object eval = null;
        try {
            if (isJsonFormat(script)) {
